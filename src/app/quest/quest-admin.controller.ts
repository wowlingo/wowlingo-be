...
export class QuestAdminController {
  ...
  async findAllQuests(): Promise<BaseResponse<AdminQuestResDto[]>> {
    ...
    const resDtos = await this.questService.makeQuestsAdmin(quests, hashtags);
  ...
  async findAllQuestItems(@Query('questId') questId?: string): Promise<BaseResponse<AdminQuestItemResDto[]>> {
    ...
    const questItemUnits1 = await this.questService.findQuestItemUnitsByQuest1QuestItemIds(questItemIds);
    const questItemUnits2 = await this.questService.findQuestItemUnitsByQuest2QuestItemIds(questItemIds);
    const questItemUnits = await this.questService.findAllQuestItemUnits();

    const resDtos = await this.questService.makeQuestItemsAdmin(questItems, questItemUnits1, questItemUnits2, questItemUnits);

    return BaseResponse.success(resDtos, '퀘스트 아이템 목록을 성공적으로 조회했습니다.');
  }
  ...
  async findAllQuestItemUnits(): Promise<BaseResponse<AdminQuestItemUnitResDto[]>> {
    ...
    const questItemUnitIds = questItemUnits.flatMap(it => it.questItemUnitId).filter((id): id is number => id !== null);
    const hashtags = await this.hashtagService.findGroupNamesByQuestItemUnits(questItemUnitIds);

    const resDtos = await this.questService.makeQuestItemUnitsAdmin(questItemUnits, hashtags);

    return BaseResponse.success(resDtos, '퀘스트 아이템 유닛 목록을 성공적으로 조회했습니다.');
  }
